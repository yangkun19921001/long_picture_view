...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    // 日志地址
    private String logPath = "big_picture";
    private String dir;
    ...
    private boolean handleException(final Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Log.i(TAG, "run开始");
                Looper.prepare();
                Toast.makeText(mContext, "应用出现错误,请重新登录!" +"\r\n"+ ex.getMessage(), Toast.LENGTH_LONG)
                        .show();
                Looper.loop();
                Log.i(TAG, "run结束");
            }
...
/*        ErrorLogBean errorLogBean = new ErrorLogBean();
        ...
        try {
            ...
            String fileName = "T01-" + time + "-" + timestamp + ".txt";
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                String path = Environment.getExternalStorageDirectory() + "/" + logPath + "/";
